...
<!--
Internal tides are internal gravity waves generated by the interaction of barotropic tides with seafloor topography. They are a primary driver of ocean mixing and play a vital role in the global energy budget. 
-->
...
<!--
![Example of plane wave fit performed. Top left: synthetic SSH data, composed of three waves, Gaussian white noise, and a coherent eddy field. Top right: extracted wave amplitudes in all compass directions (Wave 1, Wave 2, Wave 3). Bottom left: a snapshot of SWOT SSH data. Bottom right: same as top right but for SWOT data.](figure.png){#fig:example}
-->
...
The package complements existing oceanographic tools. While `dynmodes` [@klinck_dynmodes] computes theoretical vertical mode structures from stratification profiles, it does not provide the conversion factors needed to relate surface observations to full-depth energetics. PlaneWaveFit bridges this gap by including a global database of these conversion factors, precomputed from World Ocean Atlas 2023 climatology. This integration allows users to translate fitted SSH amplitudes into physical quantities (e.g., depth-integrated energy, energy flux) without additional calculations.

The package complements theoretical vertical mode solvers such as `dynmodes` [@klinck_dynmodes], which compute theoretical vertical mode structures and eigenspeeds from stratification profiles. First, for users who do not have a prescribed wavenumber for their study region, the package provides access to precomputed wavenumbers derived from global climatology, calculated using `dynmodes`. Second, once a fitted amplitude is obtained, the software provides the necessary conversion ratios—such as the ratio of depth-integrated energy to surface amplitude squared—to translate surface results into full-depth energy budgets. This modularity allows users to either perform a standalone 2D fit or utilize the included physical parameters to move from sea surface height to physical energy flux.
-->
...
These ratios enable the direct translation of surface-fitting results into full-depth energetics. For example, a 5 cm fitted surface amplitude can be immediately converted into depth-integrated energy without requiring the user to perform manual vertical integration of stratification profiles. The physical derivation of these factors is detailed in (remember to put the doi of preprint after it is released).
...
from planewavefit import utils  
...
PlaneWaveFit provides a standardized, open-source implementation of 2D plane-wave fitting. The software has been applied to SWOT data in the Southern Ocean [cite my paper], successfully extracting coherent tidal beams and refraction patterns despite strong mesoscale variability. By combining fitted surface amplitudes with the precomputed conversion factors, users can estimate depth-integrated energy and energy flux directly from SSH observations. This enables mapping of internal tide energy pathways and identifying regions where tidal energy flux decays and supports turbulent mixing.
