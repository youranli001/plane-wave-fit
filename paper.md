...
![Example of plane wave fit performed. Top left: synthetic SSH data, composed of three waves, guassian white noise and a coherent eddy field. Top right: extracted wave amplitudes in all compass directions.(wave 1 wave 2 wave 3). Bottom left: a snapshot of SWOT SSH data. Bottom right: same as top right..\label{fig:example}](figure.png)
...
from planewavefit import utils  # Helper functions for fitting and coordinate conversion
...
# Load the dataset
ds = xr.open_dataset(data_dir / ssha_fn)
...
distX, distY = utils.lonlat2xy(ds.longitude.values, ds.latitude.values, 
                               lon_example, lat_example)

# Reshape data into (Time, Y, X) dimensions: (num_cycles, num_lines, num_pixels)
ssha = ds['ssha'].values
